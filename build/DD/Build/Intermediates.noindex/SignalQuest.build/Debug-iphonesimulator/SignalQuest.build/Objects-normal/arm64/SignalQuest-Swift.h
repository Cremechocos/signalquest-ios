...
#ifndef SIGNALQUEST_SWIFT_H
#define SIGNALQUEST_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="SignalQuest",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
@class UIApplication;
@class NSData;
/// We bridge UIApplicationDelegate via SwiftUI’s <code>@UIApplicationDelegateAdaptor</code>
/// to receive the APNs device token. The push service is injected from the
/// SwiftUI environment once it’s available.
SWIFT_CLASS("_TtC11SignalQuest11AppDelegate")
...
/// <ul>
///   <li>
///     <code>POST /api/user/voip-token</code> <code>{ voipToken, platform: "ios" }</code> to store the
///     VoIP token (separate APNs topic <code><bundleid>.voip</code>).
///   </li>
///   <li>
///     On call initiation, the server sends a VoIP push whose payload contains at
///     least <code>callId</code>/<code>conversationId</code>/<code>caller</code>/<code>mode</code> so we can report it.
...
///   </li>
/// </ul>
SWIFT_CLASS("_TtC11SignalQuest11CallManager")
...
@class PKPushPayload;
@interface CallManager (SWIFT_EXTENSION(SignalQuest)) <PKPushRegistryDelegate>
...
@interface CallManager (SWIFT_EXTENSION(SignalQuest)) <CXProviderDelegate>
...
@class CLLocation;
SWIFT_CLASS("_TtC11SignalQuest15LocationService")
...
SWIFT_CLASS("_TtC11SignalQuest15MockURLProtocol")
...
@class NSString;
SWIFT_CLASS("_TtC11SignalQuest18NetworkPathMonitor")
@interface NetworkPathMonitor : NSObject <CTTelephonyNetworkInfoDelegate>
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
- (void)dataServiceIdentifierDidChange:(NSString * _Nonnull)identifier;
@end
...
/// <code>/api/user/fcm-token</code>.
SWIFT_CLASS("_TtC11SignalQuest23PushNotificationService")
...
@interface PushNotificationService (SWIFT_EXTENSION(SignalQuest)) <UNUserNotificationCenterDelegate>
- (void)userNotificationCenter:(UNUserNotificationCenter * _Nonnull)center willPresentNotification:(UNNotification * _Nonnull)notification withCompletionHandler:(void (^ _Nonnull)(UNNotificationPresentationOptions))completionHandler;
/// Handles a notification tap. We extract the identifiers off the (non-Sendable)
/// payload here, then hand only <code>String?</code> values to the MainActor router so a
