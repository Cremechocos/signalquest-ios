...
`timeout-one` garde la même grille mais ne transmet aucun en-tête ni octet pour la tuile centrale. Le serveur attend la fermeture du client, avec une limite de sécurité de 180 secondes, et journalise la durée et `client-disconnected` avec statut0 : ce n'est pas une réponse HTTP 503. Le test `testTimedOutSpeedtestTilePreservesOtherPointsAndRecoversThroughRetry` exige une fermeture après le délai réseau, les deux points reçus conservés et les cinq points rétablis après Réessayer. Une expiration de l'attente XCTest ne valide pas ce scénario.

Sur iPhone physique, utiliser uniquement la Beta de recette, une origine USB appairée vérifiée et un proxy local qui conserve la fixture sur loopback. Le runner peut fournir `SQ_MAP_FIXTURE_ORIGIN` (origine compilée identique), `SQ_MAP_HTTP_QA=1` et `SQ_MAP_PHYSICAL_QA=paired-usb-verified`. Il doit vérifier le bundle Beta, les deux origines et le chemin de routage avant le test, puis restaurer le proxy de recette après celui-ci. Le test `APIClientTests/testPhysicalSilentMapTransportRespectsRequestDeadline`, opt-in avec `SQ_MAP_TRANSPORT_QA=paired-usb-verified`, isole le transport réel et exige une erreur avant sa propre annulation de sécurité à 45 secondes. Conserver séparément traces de transport, rendu et tests substitués.
